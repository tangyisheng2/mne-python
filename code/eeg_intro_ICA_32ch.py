...
def main(tester: str):
    ...
    for event_index in range(0, events.__len__() - 1):
        epochs.append(
            mne.Epochs(raw, [events[event_index, :]], tmin=0, tmax=instance_len_in_each_event[event_index],
                       baseline=(0, 0),
                       preload=True))
    # print(epochs)
    ...
    #                                       columns=channel_column)
    np.savetxt(f'{"../EEGData/"}{tester}{".csv"}', result, delimiter=',')


if __name__ == '__main__':
    tester_list = utils_Eason.batch_process.list_tester('../EEGData')
